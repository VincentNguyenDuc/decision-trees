...
class DecisionTree:
    ...
    def fit(self, data, target):
        ...
        if self.depth <= self.max_depth:
            ...
            self.impurity_score = self.__calculate_impurity_score(
    ...
    def __find_best_split(self):
        ...
        for col in self.independent:
            ...
            if not best_split or best_split["information_gain"] < information_gain:
                best_split = {"split": split, "col": col,
    ...
    def __find_best_split_for_column(self, col):
        ...
        for val in unique_values:
            ...
            left_impurity = self.__calculate_impurity_score(
                left_data[self.target])
            right_impurity = self.__calculate_impurity_score(
                right_data[self.target])
    ...
    def __validate_data(self):
        non_numeric_columns = self.data[self.independent].select_dtypes(
            include=['category', 'object', 'bool']).columns.tolist()
        ...
        if (len(self.data[self.target].cat.categories) != 2):
            raise RuntimeError(
                "Implementation is only for Binary Classification")
